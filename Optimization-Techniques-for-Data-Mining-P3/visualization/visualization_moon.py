...
# Load the blob data
sampled_moon_data = load_blob_data('../data/moon_data.txt')

# Read the cluster assignments
cluster_assignments = read_cluster_assignments('../output/output_moon.txt')
...
sampled_moon_data['Cluster'] = mapped_clusters
...
for i, color in enumerate(colors):
    plt.scatter(sampled_moon_data[sampled_moon_data['Cluster'] == i].iloc[:, 0],
                sampled_moon_data[sampled_moon_data['Cluster'] == i].iloc[:, 1],
                c=color, label=f'Cluster {i+1}', marker='o')

plt.title('Moon Data Clustering')
plt.xlabel('Feature 1')
plt.ylabel('Feature 2')
plt.legend()
# Save the figure
plt.savefig('../output/moon_clusters.png', format='png', dpi=300)
