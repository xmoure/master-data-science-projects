...
```{r}
dd <- read.table("Playstore-Clustered.csv",header=T, sep=",", dec='.',stringsAsFactors=TRUE)
dd$Minimum.Android <- as.factor(dd$Minimum.Android)
names(dd)


summary(dd)
dd$Rating.Count <- exp(dd$Rating.Count)
dd$Size <- exp(dd$Size)
dd$Installs <- exp(dd$Installs)
...
for(k in 1:K){
  if (is.numeric(dades[,k])){ 
    ...
    if(class(dd[,k])=="Date"){
      ...
      paleta<-rainbow(length(levels(dades[,k])))
      barplot(prop.table(table(dades[,k], as.factor(P)),2), beside=FALSE,col=paleta,main=paste("Barplot of ",names(dades)[k]))
      legend("topright",levels(as.factor(dades[,k])),pch=1,cex=0.5, col=paleta)
      
      paleta<-rainbow(length(levels(as.factor(P))))
      barplot(prop.table(table(as.factor(P),dades[,k]),2), beside=FALSE,col=paleta,main=paste("Barplot of ",names(dades)[k]))
      legend("topright",levels(as.factor(P)),pch=1,cex=0.5, col=paleta)
      
      #diagrames de barres adosades
      barplot(prop.table(table(dades[,k], as.factor(P)),2), beside=TRUE,col=paleta)
      legend("topright",levels(as.factor(dades[,k])),pch=1,cex=0.5, col=paleta)
