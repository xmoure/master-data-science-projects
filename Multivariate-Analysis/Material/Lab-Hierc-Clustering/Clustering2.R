...
names <- c('Cultivar','Alcohol','Malic_acid' , 'Ash', 'Alcalinity_ash', 'Magnesium', 'Total_phenols', 
...
###############
fviz_dend(x = fit, k = 6, cex = 0.6) +
  geom_hline(yintercept = 2500, linetype = "dashed") +
  labs(title = "Hierar.Clustering",
       subtitle = "Euclidean Distance - Method Ward - K=6")
...
centroides <- data.frame(data%>%dplyr::group_by(clusters)%>%dplyr::summarise(Proline=mean(Proline),Alcohol=mean(Alcohol),Flavanoids=mean(Flavanoids),
              Color_intensity=mean(Color_intensity), Magnesium=mean(Magnesium)))
