...
public class Exercise_1 {
    static int Superstep  = 0;
    private static class VProg extends AbstractFunction3<Long,Integer,Integer,Integer> implements Serializable {
        @Override
        public Integer apply(Long vertexID, Integer vertexValue, Integer message) {
            if (message == Integer.MAX_VALUE) {             // superstep 0
                System.out.println("Superstep:: "+ Superstep + " vertexiid " + vertexID + " with message: " +  message + " returned vertex value " + vertexValue);
                return vertexValue;
            } else {
                // superstep > 0
                Superstep = Superstep + 1;
                System.out.println("Superstep:: "+ Superstep + " vertexiid " + vertexID + " with message: " +  message + " returned value " + Math.max(vertexValue,message));
        ...
        public Iterator<Tuple2<Object, Integer>> apply(EdgeTriplet<Integer, Integer> triplet) {
            ...
            if (sourceVertex._2 <= dstVertex._2) {   // source vertex value is smaller than dst vertex?
                ...
                return JavaConverters.asScalaIteratorConverter(
                        new ArrayList<Tuple2<Object,Integer>>().iterator()).asScala();
            } else {
                // propagate source vertex value
                System.out.println("Sending messages::Superstep:: "+ Superstep + " vertex source "  + sourceVertex._1 + " send message to " +  dstVertex._1 + " the message " + sourceVertex._2);
