...
```{r}
setwd("C:/Users/ximena.moure/Desktop")
...
#odds severe accidents vs mild accidents
oddprob<-prob/(1-prob);oddprob
oddprob<-sum(acc2$Severity.Severe)/sum(acc2$Severity.Mild);oddprob
log(oddprob)

#null model
#we have grouped data so we need to do Severity.Sever, Severity.Mild
#on the results:
#deviance measure of discrepancy
#null deviance: discrepancy between my data and the model
...
#p-value 0.33, so the null hypothesis can´t be rejected, so the two models account for the same
anova(m3,m4,test="Chisq")
waldtest(m3,m4,test="Chisq")
